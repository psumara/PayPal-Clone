...
document.querySelectorAll('.signup').forEach(signupBtn=>{
    signupBtn.addEventListener('click', () => {
        ...
        document.querySelector('.checkbox-personal').checked = true
    })
})

document.querySelectorAll('.signup-business').forEach(signupBtn=>{
    signupBtn.addEventListener('click', () => {
        document.querySelector('.front-page').style.display='none'
        document.querySelector('.login-page').style.display='none'
        document.querySelector('.signup-page').style.display='flex'
        document.querySelector('.checkbox-business').checked = true
    })
})
// End of Pages
// Navigation
...
if(window.innerWidth < 1000){
    ...
    menuIcon.addEventListener('click', () => {
        ...
        if(!navbar.classList.contains('change')){
            ...
            document.querySelectorAll('.nav-dropdown-2').forEach(dropdown => {
                dropdown.style.left='-20rem'
            })
        }
    })
